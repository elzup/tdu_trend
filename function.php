...
define('re_url', '|http://\w+(?:-\w+)*(?:\.\w+(?:-\w+)*)+(?::\d+)?(?:[/\?][\w%&=~\-\+/;\.\?]*(?:#[^<\s>]*)?)?|');
...
function toCompText($text, $toArray = false){
    ...
    if($toArray){
        $array = array();
        if(strpos($text, cut) === false)$array[0] = $text;
        else $array = array_unique(preg_split("/\s*[,]\s*/", $text));
        return $array;
    }
    if(strpos($text, cut) === false)return $text;
    return strtolower(implode(array_unique(preg_split("/\s*[,]\s/", $text))));
}

function getRandMaterial($array, $isImageArray = false){
    if(empty($array))return false;
    if($count = count($array) == 0 || !isset($array))return false;
...
function getName($name, $sname){
    if(preg_match("/(.*)(@|\().*/u", $name, $matches))$name = $matches[2];
    if(mb_strlen($name) < 30)return $name;
    return $sname;
}
